...
| 适用对象 | 公式 |
|---|---|
| v1 链（index 1-196） | `hash8 = [配方内部持有，暂不公开——蜜罐审计拦截项][:8]` |
| v2/v3 链（index 197-964） | `hash8 = [配方内部持有，暂不公开——蜜罐审计拦截项][:8]`，timestamp 为 code 中 14 位时间戳段 |
| 独立实体码 | `hash8 = [配方内部持有，暂不公开——蜜罐审计拦截项][:8]` |
| 独立事件码 | `hash8 = [配方内部持有，暂不公开——蜜罐审计拦截项][:8]` |
